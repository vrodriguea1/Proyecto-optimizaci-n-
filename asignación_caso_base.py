# -*- coding: utf-8 -*-
"""Asignación caso base.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/18Y0og4RMXM7g77JMBq60sMJJ6hHe_LTg
"""
...
for i in range(cursos):
  solver.Add(solver.Sum([X[i,j] for j in range(profesores)]) <= 1)

for j in range(profesores):
  solver.Add(solver.Sum([X[i,j]*G[j,0]*(1-F[i,j]) for i in range(cursos)]) <= 1)

for j in range(profesores):
  solver.Add(3*solver.Sum([X[i,j]*G[j,0]*B[i,j] for i in range(cursos)]) == I[j,0])

for j in range(profesores):
  solver.Add(3*solver.Sum([X[i,j]*(1-G[j,0])*B[i,j] for i in range(cursos)]) <= I[j,0])
